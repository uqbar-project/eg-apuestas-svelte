<script lang="ts">
	...
	let apuestaPojo = $state({
		monto: 0,
		fecha: new Date(),
		tipoApuesta: PLENO,
		valorApostado: 1 as string | number
	})
...
<form class="form">
	<div class="row">
		...
		<input
			type="date"
			onchange={() => {
				apuestaPojo.fecha = dayjs(fechaApuesta).toDate()
			}}
	...
	<div class="row">
		...
		<select
			...
			name="tipoApuesta"
			class="form-control"
			bind:value={apuestaPojo.tipoApuesta}
			required={true}
			onchange={() => {
				apuestaPojo.valorApostado = ''
			}}
		>
	...
	<div class="row">
		...
		<select
			...
			data-testid="apuesta"
			class="form-control"
			bind:value={apuestaPojo.valorApostado}
			disabled={!apuestaPojo.tipoApuesta?.valoresAApostar}
			required={true}
		>
	...
	<div class="botonera">
		<button class="btn-primary" data-testid="btnApuesta" onclick={apostar} type="submit"
			>Apostar</button
		>
	</div>
	<div class="row">
